...
export default async function handler(req: any, res: any) {
  ...
  if (req.method === 'GET') {
    ...
    return sendJson(res, 200, { success: true, stats, totalEvents: events.length, recentEvents: events.slice(0, 25) });
  }
  ...
  if (req.method === 'POST') {
    try {
      ...
      return sendJson(res, 500, { success: false, error: err.message || 'Failed to log event' });
    }
  }
