...
// src/core/growth/qr-menu.engine.ts
var QrMenuEngine = class {
  /**
   * Sanitizes and generates a URL-safe lowercase slug from a business name.
   */
  static generateSlug(businessName, existingSlugs = []) {
    let slug = businessName.toLowerCase().trim().replace(/[^\w\s-]/g, "").replace(/[\s_-]+/g, "-").replace(/^-+|-+$/g, "");
    if (!slug) {
   ...
   */
  static createDefaultMenu(restaurantId) {
    const cat1Id = `cat-${restaurantId}-1`;
    const cat2Id = `cat-${restaurantId}-2`;
    const cat3Id = `cat-${restaurantId}-3`;
    const categories = [
      { id: cat1Id, restaurantId, name: "Starters & Appetizers", sortOrder: 1, createdAt: (/* @__PURE__ */ new Date()).toISOString() },
      { id: cat2Id, restaurantId, name: "Main Course", sortOrder: 2, createdAt: (/* @__PURE__ */ new Date()).toISOString() },
      { id: cat3Id, restaurantId, name: "Beverages & Desserts", sortOrder: 3, createdAt: (/* @__PURE__ */ new Date()).toISOString() }
    ];
    const items = [
      {
   ...
   */
  static generateQrCodeSvg(text, size = 240) {
      ...
      for (let c = 0; c < modulesCount; c++) {
        if (!isFinderPattern(r, c)) {
          if (r === 6 || c === 6) {
            ...
            const cellHash = Math.abs(Math.sin(r * 31 + c * 17 + hash) * 1e4);
  ...
  if (req.method === "POST") {
    try {
      ...
      if (action === "item" || body.action === "item") {
        ...
        if (!restaurantId || !categoryId || !name || price === void 0) {
      ...
      const generatedSlug = customSlug ? QrMenuEngine.generateSlug(customSlug, existingSlugs) : QrMenuEngine.generateSlug(businessName, existingSlugs);
      const restaurant = db.saveRestaurant({
        businessName: businessName.trim(),
        phone: phone.trim(),
        city: city ? city.trim() : "India",
        logoUrl: logoUrl ? logoUrl.trim() : void 0,
        slug: generatedSlug,
        isPublished: true
      });
