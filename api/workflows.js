...
var DatabaseService = class _DatabaseService {
  ...
  constructor(customPath) {
    if (customPath) {
      ...
      this.filePath = path2.resolve("/tmp", "primesoul_data.json");
    } else {
      this.filePath = path2.resolve(process.cwd(), "primesoul_data.json");
    }
  ...
  loadData() {
    ...
    const rootPath = path2.resolve(process.cwd(), "primesoul_data.json");
    if (this.filePath !== rootPath && fs2.existsSync(rootPath)) {
      try {
        const raw = fs2.readFileSync(rootPath, "utf-8");
        const parsed = JSON.parse(raw);
  ...
  saveData(data = this.data) {
    try {
      fs2.writeFileSync(this.filePath, JSON.stringify(data, null, 2), "utf-8");
    } catch (err) {
